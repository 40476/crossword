...
function show_help(){alert(`Select a row or column to get word suggestions,\nthen click on a suggestion to add it.\n\nDouble-click a letter to remove that word.\nDouble-click an empty box to enter a hint.`);}
...
function delete_crossword_row(y) {
    if (crossword_height() <= 2) {
        alert("needs more than 2 lines!");
        return;
    }
    for (let x = 0; x < crossword_width(); x++) {
        if (!cell_isempty(crossword_cell(x, y))) {
            alert("row is not empty! Remove all letters and clues!");
            return;
        }
    }
...
function delete_crossword_column(x) {
    if (crossword_width() <= 2) {
        alert("needs more than 2 columns!");
        return;
    }
    for (let y = 0; y < crossword_height(); y++) {
        if (!cell_isempty(crossword_cell(x, y))) {
            alert("collumn is not empty! Remove all letters and clues!");
            return;
        }
    }
    for (let y = 0; y < crossword_height(); y++) {
